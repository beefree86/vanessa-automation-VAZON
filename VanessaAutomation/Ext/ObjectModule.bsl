...
Функция ПодставитьЗначенияПеременныхВШаг(Знач Стр, ДанныеШага, ПодставлятьЗначения = Истина, СписокИсключений = Неопределено)
	...
	Иначе
		...
		Если ДанныеШага.Свойство("ПеременныеДляСохраненияВШаг") Тогда
...
Процедура ЗаписатьВыполнениеШагаАллюр2(МассивШагов, ДеревоТестов, РезультатыПрохождения, СоответствиеГрупп, МассивXMLОтчетаAllure, СтруктураВыгрузкиСценария, ДопПараметрыАллюр2)
	...
	Если РезультатыПрохождения.Свойство("ПеременныеДляСохраненияВШаг") Тогда
		...
		Шаг.name = ПодставитьЗначенияПеременныхВШаг(Строка(РезультатыПрохождения.Имя), РезультатыПрохождения, ДопПараметрыАллюр2.ПодставлятьВОтчетеAllureЗначенияПеременных, ДопПараметрыАллюр2.СписокИсключаемыхПеременныхAllure);
	КонецЕсли;	 
	...
	Если СтрокаВыполнения <> Неопределено Тогда	
		...
		Для каждого Элемент Из СтрокаВыполнения.ЗначенияПараметров Цикл
			...
			Если ДопПараметрыАллюр2.ПодставлятьВОтчетеAllureЗначенияПеременных Тогда
			    Значение = ПодставитьЗначенияПеременныхВШаг(ТекстПараметра, РезультатыПрохождения, ДопПараметрыАллюр2.ПодставлятьВОтчетеAllureЗначенияПеременных, ДопПараметрыАллюр2.СписокИсключаемыхПеременныхAllure);
			Иначе	
		...
		Если СтрокаВыполнения.ШагСПараметрамиВТаблице = Истина Тогда
			...
			Для каждого СтрокиТаблицы Из ПодчиненныеСтроки Цикл
				...
				Для каждого Колонка Из СтрокиТаблицы.ПараметрыТаблицы Цикл
				    ...
				    Если ДопПараметрыАллюр2.ПодставлятьВОтчетеAllureЗначенияПеременных Тогда
				        // Проверяем, является ли значение строкой и содержит ли оно переменные
				        Если ТипЗнч(Значение) = Тип("Строка") Тогда
				            Значение = ПодставитьЗначенияПеременныхВШаг(Значение, РезультатыПрохождения, ДопПараметрыАллюр2.ПодставлятьВОтчетеAllureЗначенияПеременных, ДопПараметрыАллюр2.СписокИсключаемыхПеременныхAllure);
				        КонецЕсли;
				    КонецЕсли;	 
...
Процедура ЗаполитьПараметрыДляAllureTestOps(СтруктураВыгрузки, СтрокаСценария, ДопПараметрыАллюр2)
	...
	Если Прав(ОтносительныйКаталогФичи, 8) = ".feature" Тогда
		ОтносительныйКаталогФичи = Лев(ОтносительныйКаталогФичи, СтрДлина(ОтносительныйКаталогФичи) - 8);
	КонецЕсли;
	...
	Для каждого СтрСтроки Из Дерево.Строки Цикл
		...
		ИначеЕсли (СтрСтроки.Тип = "Сценарий") ИЛИ (СтрСтроки.Тип = "Пример") Тогда
			...
			Иначе
				...
				// дополняем имя значениями параметров через "_"
				Если СтруктураВыгрузки.Свойство("parameters") И СтруктураВыгрузки.parameters.Количество() > 0 Тогда
					...
					СтруктураВыгрузки.name = СтруктураВыгрузки.name + "_" + СтрокаПараметров; 
				КонецЕсли;
