...
Процедура ЗаполитьПараметрыДляAllureTestOps(СтруктураВыгрузки, СтрокаСценария, ДопПараметрыАллюр2)
	...
	// Явное приведение путей к единому виду
	ПутьКФичеУниверсальный = НРег(СтрЗаменить(ПутьКФиче, "\", "/"));
	КаталогПроектаУниверсальный = НРег(СтрЗаменить(ДопПараметрыАллюр2.КаталогПроекта, "\", "/"));
	...
	ОтносительныйПутьФичи = СокрЛП(Сред(ПутьКФичеУниверсальный, СтрДлина(КаталогПроектаУниверсальный) + 1));
	
	Если Прав(ОтносительныйПутьФичи, 8) = ".feature" Тогда
		ОтносительныйПутьФичи = Лев(ОтносительныйПутьФичи, СтрДлина(ОтносительныйПутьФичи) - 8);
	КонецЕсли;

	fullName = ОтносительныйПутьФичи + "_" + СтруктураВыгрузки.name;
