...
Процедура ДобавитьТекстРасширенныеДействия(Текст, Параметры, ОбъектМетаданных, Группа = Ложь)
	...
	Если Параметры.ВидОбъектаЕЧ = "Документ" Тогда
		...
		Иначе
			ТекстУсловие = "";
		КонецЕсли;
		// Добавляем отбор по дате, если задана
		Если Параметры.Свойство("НачальнаяДатаДляПодбораДокументов")
			И ЗначениеЗаполнено(Параметры.НачальнаяДатаДляПодбораДокументов) Тогда
			Если Параметры.ЯзыкШагов = "ru" Тогда
				ТекстУсловие = ТекстУсловие + " И ТекДанныеИсточник.Дата >= &НачальнаяДатаДляПодбораДокументов";
			Иначе
				ТекстУсловие = ТекстУсловие + " And CurrentDataSource.Date >= &НачальнаяДатаДляПодбораДокументов";
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли Параметры.ВидОбъектаЕЧ = "Справочник"
	    ...
	    // только для объектов, у которых есть поле Предопределенный
		Если Параметры.ЯзыкШагов = "ru" Тогда
	...
	Если Параметры.ТолькоВведенныеОбъекты Тогда
		...
		Если Параметры.Свойство("НачальнаяДатаДляПодбораДокументов") И ЗначениеЗаполнено(Параметры.НачальнаяДатаДляПодбораДокументов) Тогда
			Запрос.УстановитьПараметр("НачальнаяДатаДляПодбораДокументов", Параметры.НачальнаяДатаДляПодбораДокументов);
		КонецЕсли;
	...
	Если Параметры.ЯзыкШагов = "ru" Тогда 
		...
		Если Параметры.Свойство("НачальнаяДатаДляПодбораДокументов") И ЗначениеЗаполнено(Параметры.НачальнаяДатаДляПодбораДокументов) Тогда
		    ...
		    ЛитералДаты = Формат(Параметры.НачальнаяДатаДляПодбораДокументов, "ДФ=yyyy-MM-ddTHH:mm:ss");

		    СтрокаУстановкиПараметра =
		    "
		    |           Запрос.УстановитьПараметр(""НачальнаяДата"", Дата('" + ЛитералДаты + "'));";
		КонецЕсли;
	...
	Иначе
		...
		Если Параметры.Свойство("НачальнаяДатаДляПодбораДокументов") И ЗначениеЗаполнено(Параметры.НачальнаяДатаДляПодбораДокументов) Тогда
		    ...
		    СтрокаУстановкиПараметра =
		    "
		    |           | 'Query.SetParameter(""НачальнаяДата"", Date('" + ЛитералДаты + "'));' |";
		КонецЕсли;
