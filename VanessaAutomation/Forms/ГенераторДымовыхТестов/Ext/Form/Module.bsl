...
Процедура СформироватьФайлыСценариевТестирования()
	...
	Для Каждого СтрокаНастройки ИЗ ТаблицаНастроек Цикл
		...
		Для СчТип = 1 По СтруктураПараметров.СоотвестствиеНастроек.Количество() Цикл
			
			ТипТеста = СтруктураПараметров.СоотвестствиеНастроек.Получить(СчТип);
			Если СтрокаНастройки[ТипТеста] Тогда
				...
				Если Текст.КоличествоСтрок() > 0 Тогда
					...
					// Запишем файл
					ПутьЗаписи = КаталогВыходныхФайлов + "\" + СчОбъект + СчТип + "_" + СтруктураПараметров.ПредставлениеИменаМетаданных[СтруктураПараметров.ВидОбъектаМетаданных] 
						+ "_" + СтруктураПараметров.ПредставлениеТипыТеста[ТипТеста] + ".feature";
...
Функция СтруктураПараметровГенерацииСценариев(ЯзыкШагов)
	...
	СтруктураПараметров.Вставить("СоотвестствиеНастроек", СоотвестствиеНастроек);
	
	СтруктураПараметров.Вставить("СчТип", 1);   // Вариант по-умолчанию
	СтруктураПараметров.Вставить("ТипТеста", СоотвестствиеНастроек.Получить(1));
...
Процедура ЗаполнитьИсключения()
	...
	ТипыОбъектовМетаданныхНаАнлийском = ПредставлениеИменаМетаданных("en");
	...
	Для Каждого СтрокаНастройки ИЗ ТаблицаНастроек Цикл
		...
		Если НайденныеФайлы.Количество() = 0 Тогда
			...
			НайденныеФайлы = НайтиФайлы(КаталогФайловИсключений, ТипыОбъектовМетаданныхНаАнлийском[СтрокаНастройки.ОбъектМетаданных] + "*.txt", Истина);   
			ЗаполнитьТаблицуИсключенийПоТипуОбъектаМетаданных(СтрокаНастройки.ОбъектМетаданных, НайденныеФайлы);
...
&НаКлиенте
Процедура ОбновитьФичаФайлы(Команда)
...
Процедура ЗаполнитьТаблицуНастроекПоУмолчанию()
	...
	Для Каждого ТипМетаданных из ТипыМетаданных Цикл
		...
		Если ТипМетаданных.Ключ = "Справочники" Тогда
		...
		ИначеЕсли ТипМетаданных.Ключ = "Документы" Тогда
			НСтрокаНастроек.ФормаОбъекта = Истина;
			НСтрокаНастроек.ФормаСписка = Истина;
			НСтрокаНастроек.Запись = Истина;
			НСтрокаНастроек.Копирование = Истина;
			НСтрокаНастроек.ВводНаОсновании = Истина;
			НСтрокаНастроек.Печать = Истина;
		ИначеЕсли ТипМетаданных.Ключ = "ЖурналыДокументов" Тогда
			НСтрокаНастроек.ФормаОбъекта = Истина;
		ИначеЕсли ТипМетаданных.Ключ = "ПланыВидовХарактеристик" Тогда
			НСтрокаНастроек.ФормаОбъекта = Истина;
			НСтрокаНастроек.ФормаСписка = Истина;
			НСтрокаНастроек.Запись = Истина;
			НСтрокаНастроек.Копирование = Истина
		ИначеЕсли ТипМетаданных.Ключ = "ПланыСчетов" Тогда
			НСтрокаНастроек.ФормаОбъекта = Истина;
			НСтрокаНастроек.ФормаСписка = Истина;
			НСтрокаНастроек.Запись = Истина;
		ИначеЕсли ТипМетаданных.Ключ = "ПланыВидовРасчета" Тогда
			НСтрокаНастроек.ФормаОбъекта = Истина;
			НСтрокаНастроек.ФормаСписка = Истина;
			НСтрокаНастроек.Запись = Истина;
			НСтрокаНастроек.Копирование = Истина;
		ИначеЕсли ТипМетаданных.Ключ = "Отчеты" Тогда
			НСтрокаНастроек.ФормаОбъекта = Истина;
		ИначеЕсли ТипМетаданных.Ключ = "Обработки" Тогда
			НСтрокаНастроек.ФормаОбъекта = Истина;
		ИначеЕсли ТипМетаданных.Ключ = "РегистрыСведений" Тогда
			НСтрокаНастроек.ФормаОбъекта = Истина;
			НСтрокаНастроек.ФормаСписка = Истина;
		ИначеЕсли ТипМетаданных.Ключ = "РегистрыНакопления" Тогда
			НСтрокаНастроек.ФормаСписка = Истина;
		ИначеЕсли ТипМетаданных.Ключ = "РегистрыБухгалтерии" Тогда
			НСтрокаНастроек.ФормаСписка = Истина;
		ИначеЕсли ТипМетаданных.Ключ = "РегистрыРасчета" Тогда
			НСтрокаНастроек.ФормаСписка = Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьТипыМетаданных(Язык)
	...
	ТипыМетаданных.Вставить("Справочники", ДанныеПоМетаданному(ПредсталениеМетаданных.Справочники, 
		ПредставлениеЕдиницыМетаданных.Справочник, БиблиотекаКартинок.Справочник));
	ТипыМетаданных.Вставить("Документы", ДанныеПоМетаданному(ПредсталениеМетаданных.Документы, 
		ПредставлениеЕдиницыМетаданных.Документ, БиблиотекаКартинок.Документ));
	ТипыМетаданных.Вставить("ПланыВидовХарактеристик", ДанныеПоМетаданному(ПредсталениеМетаданных.ПланыВидовХарактеристик, 
		ПредставлениеЕдиницыМетаданных.ПланВидовХарактеристик, БиблиотекаКартинок.ПланВидовХарактеристик));
	ТипыМетаданных.Вставить("ПланыСчетов", ДанныеПоМетаданному(ПредсталениеМетаданных.ПланыСчетов, 
		ПредставлениеЕдиницыМетаданных.ПланСчетов, БиблиотекаКартинок.ПланСчетов));
	ТипыМетаданных.Вставить("ПланыВидовРасчета", ДанныеПоМетаданному(ПредсталениеМетаданных.ПланыВидовРасчета, 
		ПредставлениеЕдиницыМетаданных.ПланВидовРасчета, БиблиотекаКартинок.ПланВидовРасчета));
	ТипыМетаданных.Вставить("ЖурналыДокументов", ДанныеПоМетаданному(ПредсталениеМетаданных.ЖурналыДокументов, 
		ПредставлениеЕдиницыМетаданных.ЖурналДокументов, БиблиотекаКартинок.ЖурналДокументов));
	ТипыМетаданных.Вставить("Отчеты", ДанныеПоМетаданному(ПредсталениеМетаданных.Отчеты, 
		ПредставлениеЕдиницыМетаданных.Отчет, БиблиотекаКартинок.Отчет));
	ТипыМетаданных.Вставить("Обработки", ДанныеПоМетаданному(ПредсталениеМетаданных.Обработки, 
		ПредставлениеЕдиницыМетаданных.Обработка, БиблиотекаКартинок.Обработка));
	ТипыМетаданных.Вставить("РегистрыСведений", ДанныеПоМетаданному(ПредсталениеМетаданных.РегистрыСведений, 
		ПредставлениеЕдиницыМетаданных.РегистрСведений, БиблиотекаКартинок.РегистрСведений));
	ТипыМетаданных.Вставить("РегистрыНакопления", ДанныеПоМетаданному(ПредсталениеМетаданных.РегистрыНакопления, 
		ПредставлениеЕдиницыМетаданных.РегистрНакопления, БиблиотекаКартинок.РегистрНакопления));
	ТипыМетаданных.Вставить("РегистрыБухгалтерии", ДанныеПоМетаданному(ПредсталениеМетаданных.РегистрыБухгалтерии, 
		ПредставлениеЕдиницыМетаданных.РегистрБухгалтерии, БиблиотекаКартинок.РегистрБухгалтерии));
	ТипыМетаданных.Вставить("РегистрыРасчета", ДанныеПоМетаданному(ПредсталениеМетаданных.РегистрыРасчета, 
...
Функция ПредставлениеЕдиницыМетаданных(Язык)
	...
	Если Язык = "en" Тогда
		ВозвращемыеДанные.Вставить("Справочник",				"Catalog");
		ВозвращемыеДанные.Вставить("Документ",					"Document");
		ВозвращемыеДанные.Вставить("ЖурналДокументов",			"DocumentJournals");
		ВозвращемыеДанные.Вставить("ПланВидовХарактеристик",	"ChartOfCharacteristicTypes");
		ВозвращемыеДанные.Вставить("ПланСчетов",				"ChartOfAccount");
		ВозвращемыеДанные.Вставить("ПланВидовРасчета",			"ChartOfCompensationTypes");
		ВозвращемыеДанные.Вставить("Отчет",					"Reports");
		ВозвращемыеДанные.Вставить("Обработка",					"DataProcessor");
		ВозвращемыеДанные.Вставить("РегистрСведений",			"InformationRegister");
		ВозвращемыеДанные.Вставить("РегистрНакопления",		"AccumulationRegister");
		ВозвращемыеДанные.Вставить("РегистрБухгалтерии",		"AccountingRegister");
		ВозвращемыеДанные.Вставить("РегистрРасчета",			"CalculationRegister");
	Иначе
		ВозвращемыеДанные.Вставить("Справочник",				"Справочник");
		ВозвращемыеДанные.Вставить("Документ",					"Документ");
		ВозвращемыеДанные.Вставить("ЖурналДокументов",			"ЖурналДокументов");
		ВозвращемыеДанные.Вставить("ПланВидовХарактеристик",	"ПланВидовХарактеристик");
		ВозвращемыеДанные.Вставить("ПланСчетов",				"ПланСчетов");
		ВозвращемыеДанные.Вставить("ПланВидовРасчета",			"ПланВидовРасчета");
		ВозвращемыеДанные.Вставить("Отчет",					"Отчет");
		ВозвращемыеДанные.Вставить("Обработка",					"Обработка");
		ВозвращемыеДанные.Вставить("РегистрСведений",			"РегистрСведений");
		ВозвращемыеДанные.Вставить("РегистрНакопления",		"РегистрНакопления");
		ВозвращемыеДанные.Вставить("РегистрБухгалтерии",		"РегистрБухгалтерии");
		ВозвращемыеДанные.Вставить("РегистрРасчета",			"РегистрБухгалтерии");
	КонецЕсли;
...
Процедура ПодготовкаДанныхКлиент()
	...
	Для Каждого ЭлементКласс Из ЭлементыДерева Цикл
		...
		Для Каждого ЭлементКласса Из ЭлементыКласса Цикл
			...
			//СтрокиПодготовкиДанных = МодульПодготовкаИЗагрузкаДанных.MetadataList.ПолучитьЭлементы();
			//
			//Для каждого ТекСтрока Из СтрокиПодготовкиДанных Цикл
			//	
			//	СтрокиОбъектовМетаданных = ТекСтрока.ПолучитьЭлементы();
			//	Для каждого ТекОбъект Из СтрокиОбъектовМетаданных Цикл
			//		
			//		ТекОбъект.Use = Ложь;
			//		
			//		Если Не РавенОбъектуИзПодготовкаДанных(ТипыМетаданныхНаАнгл, ЭлементКласса, ТекОбъект.FullName) Тогда
			//			Продолжить;	
			//		КонецЕсли;
			//		
			//		ТекОбъект.Use = Истина;	
			//		
			//	КонецЦикла;
			//	
			//	ИдентификаторТекСтроки = ТекСтрока.ПолучитьИдентификатор();
			//	МодульПодготовкаИЗагрузкаДанных.Элементы.MetadataList.Развернуть(ИдентификаторТекСтроки);
			//	
			//КонецЦикла;
...
Функция ЭтоНетестируемыйОбъектПоИмени(ИмяОбъектаМетаданных, ВидОбъектаМетаданных)
	...
	Если Лев(ИмяОбъектаМетаданных, 7) = "Удалить" Тогда
		Результат = Истина;
	ИначеЕсли ВидОбъектаМетаданных = "Справочники" И
		(Прав(ИмяОбъектаМетаданных, 19) = "ПрисоединенныеФайлы") Тогда
		Результат = Истина;
	ИначеЕсли ВидОбъектаМетаданных = "Отчеты" И
		(Лев(ИмяОбъектаМетаданных, 29) = "РегламентированноеУведомление"
		ИЛИ Лев(ИмяОбъектаМетаданных, 23) = "РегламентированныйОтчет") Тогда
		Результат = Истина;
	ИначеЕсли ВидОбъектаМетаданных = "Обработки" И
		(Лев(ИмяОбъектаМетаданных, 6) = "Панель") Тогда
		Результат = Истина;
	ИначеЕсли ВидОбъектаМетаданных = "РегистрыСведений" И
		(Лев(ИмяОбъектаМетаданных, 17) = "СведенияРеглОтчет") Тогда
		Результат = Истина;
	Иначе
		// Без обработки.
	КонецЕсли;
