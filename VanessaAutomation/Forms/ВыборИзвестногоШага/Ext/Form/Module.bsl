...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИспользоватьMarkdownViewer Тогда 
		...
		MarkdownViewer = "C:\Develop\reps\open-source\onec-markdown-viewer\dist\index-standalone.html"; //ПолучитьНавигационнуюСсылкуИнформационнойБазы() + "/" + ПоместитьВоВременноеХранилище(ДвоичныеДанные, УникальныйИдентификатор);
	КонецЕсли;
...
Процедура ДеревоШаговПриАктивизацииСтроки(Элемент)
	...
	Если ИспользоватьMarkdownViewer Тогда
		...
		ЭлементHTML.renderMarkdown(Элемент.ТекущиеДанные.ОписаниеШага);
	КонецЕсли;
...
Процедура УстановитьВидимостьОписанийШагов(Форма)
	...
	ЭлементыФормы.ДеревоШаговПредставлениеТестаОригинал.Видимость = НЕ Форма.ИспользоватьMarkdownViewer И Форма.ПоказатьШагиНаРусском;
	ЭлементыФормы.ДеревоШаговОписаниеШагаОригинал.Видимость       = НЕ Форма.ИспользоватьMarkdownViewer И Форма.ПоказатьШагиНаРусском;
