...
5. SikuliX (устаревшее). Установить [SikuliX](https://launchpad.net/sikuli/+milestone/1.1.3) версии 1.1. (Не обязательно) используется при записи видеоинструкций не в web клиенте (в том числе и для обычных форм).
...
7. [Oscript](http://oscript.io/downloads) (Не обязательно)
    ...
    * Необходимо прописать путь в Path.
8. [ VLC 2.1.5](https://www.videolan.org/vlc/releases/2.1.5.html) (Не обязательно) (устаревшее)
...
3. Особенности
	...
	* Если вы используете масштаб в браузере отличный от 100% или масштаб экрана в ОС отличный от 100%, то позиция курсора мышки сама калибруется, при этом масштаб экрана и браузера пересчитывается автоматически, если вызвать шаг "И я активизирую текущее окно браузера" или "И я активизирую браузер и устанавливаю полноэкранный режим"
...
* Возможно использование пустой группы шагов для выполнения шагов сценария без озвучки текста диктором.

<a id="markdown-теги" name="faq"></a>

<a id="markdown-ВидеоВЦиклах" name="ВидеоВЦиклах"></a>

## **Видеоинструкции при использовании шагов в цикле (начиная с версии после 1.2.042.19)**
1. Настройки:
* "ЗаписьВидеоГруппаШаговКакШаг": true,
2. Шаги
    * Шаги находятся в списке известных шагов в разделе **Прочее -Автоинструкции-Видеовставка** с текстом диктора. 
     Шаг: Дано текст диктора "Текст" стартует запись для видео, в т.ч. без озвучки типа Дано текст диктора "*". Звездочку удаляем через словарь замен 
    * Шаги для фиксации групп и форм находятся в разделах **UI - Группы - Зафиксировать группу** и **UI - Формы - Зафиксировать форму**.
    * И для каждой строки таблицы "ИмяТаблицы" я выполняю . Используем для циклической обработки шагов из таблицы и записи при необходимости их действий в видео
    * И для каждой строки макета "ИмяЭлементаИлиИмяФайла" я выполняю. . Используем для циклической обработки шагов из Макета и записи при необходимости их действий в видео 
    * другие аналогичные шаги для цикла типа: И для каждого(-ой)...
3. Особенности
   * Используйте директиву #[autodoc.groupsteps] для группировки в цикле шагов вместо объявления группы * Группа уровня 1
   * Используйте директиву #[autodoc.ignorestep], в т.ч. перед #[autodoc.groupsteps] для минимизации "воздуха" в видео
    * Используйте теги @StopRecordVideo или @StartRecordVideo для принудительной остановки или включения записи в Видеоинструкцию, в том числе в экспортных подсценариях. В отличии от тега @recordvideo [не включает запись в видеоинструкцию] эти теги управляют записью в Видеоинструкцию. 
    Теги @StopRecordVideo или @StartRecordVideo у шага являются приоритетными по отношению к директиве #[autodoc.ignorestep] у группы шагов. Прочие шаги между подобными тегами записываются (после шага с тегом @StartRecordVideo) или останавливается запись (после шага с тегом @StopRecordVideo) без приоритета. Например, Если был шаг с тегом @StopRecordVideo и в любом месте сценария позже вставлен шаг "Дано текст диктора 'Текст озвучки'", то будет выполнена запись озвучки в видеоинструкцию.   
    Поможет более точно и оптимально выполнить запись видеоинструкции с исключением пауз и "зависаний". В качестве примера, приведено сочетание тегов, которое поможет создать экспортный подсценарий для вставки в видеоинструкцию по сути ScreenShotа
    	Например, такой: вызываем  "И Скриншот в Видео 0.5"
Сценарий: Скриншот в Видео "_пар_Пауза"
// По сути формирует Аналог скриншота в Видео . Игнорируем 1-й шаг в группе, чтобы не было его озвучки
	#[autodoc.ignorestep]
	и пауза 0.001
	@StartRecordVideo
	и пауза '[_пар_Пауза]'	
   ...
   * Рекомендуется использование экспортного сценария для управления записью шагов в видео через таблицы (макеты, файлы и т.п.)
   * В режиме отладки с записью видео выполнена "обрезка" массива шагов фичи. Список шагов для выполнения начинается с шага запуска теста типа (Выполнить с текущего шага) и до шага остановки (Затем я останавливаю...). 
   * Рекомендуется использование экспортного сценария для управления записью шагов в видео через таблицы (макеты, файлы и т.п.)
   * Дополнительно для каждой таблицы основного цикла создайте одну таблицу с параметрами для автоинструкции
   Пример. __Таблица1 содержит поля описывающие шаг работы с интерфейсом клиента, в том числе 'Инструкции', где будет указание на получение параметров из __таблицы 2 или отсутствие необходимости смены сцен записи 
        __Таблица2 может содержать дополнительные описания для инструций, например, вид спецэффекта, имя файла для видеовставки, текст озвучки   

  
[Примеры сценариев](https://github.com/Pr-Mex/vanessa-automation/tree/develop/docs/FAQ/examples) для видеоинструкций при использовании шагов в цикле
