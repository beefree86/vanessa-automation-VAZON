...
//+++ЦВ_ПР 29.09.2025 Добавление шага с параметром таблица из секции Переменные
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,
		"ЯРисуюРамкуВокругЭлементовФормыUIAutomationПоТаблице(Парам01, ИмяТабПерем,ТабПарам1, ТабПарам2, Парам05)",
		"ЯРисуюРамкуВокругЭлементовФормыUIAutomationПоТаблице",
		"И я рисую рамку вокруг элементов формы UI Automation ""PID"" по таблице ""ИмяТаблицыИзПеременных"""
		+ Символы.ПС + "	| 'Имя'   | 'Тип' |"
		+ Символы.ПС + "	| 'Поле1' | 'Тип1' |"
		+ Символы.ПС + "	| 'Поле2' | 'Тип2' |"
		+ Символы.ПС 
		...
		+ Символы.ПС + "	| 'frameDelay'   | 20         |"
		,
		"Рисует рамку вокруг элементов из таблицы в секции Переменные. В параметр PID можно передать строку ""Этот сеанс"", тогда будет использован PID текущего сеанса, в котором запущена Vanessa Automation. ИмяТаблицыИзПеременных со структурой | 'Имя1'| 'Заголовок'| 'Тип'|- если задано, то 1-я Gherkin Таблица не анализируется. Вторая Gherkin таблица является необязательной. В ней передаются параметры отрисовки прямоугольника",
		"Прочее.UI Automation.Эффекты");
//---ЦВ_ПР 29.09.2025
...
//+++ ЦВ_ПР 29.09.2025 Новый шаг работает с таблицей переменных аналогичной структуры с Gherkin таблицей и первичным ключом

&НаКлиенте
//И я рисую рамку вокруг элементов формы UI Automation "ЭтотСеанс" по таблице "ИмяТаблицыИзПеременных"
//@ЯРисуюРамкуВокругЭлементовФормыUIAutomationПоТаблице(Парам01,ИмяТабПерем,ТабПарам,Парам04,Парам05)
Функция ЯРисуюРамкуВокругЭлементовФормыUIAutomationПоТаблице(Знач PID,ИмяТаблицыИзПеременных = Неопределено, ТаблицаЭлементов, ПараметрыОтрисовки = Неопределено, ОбработчикПродолжения = Неопределено) Экспорт  
...
//+++ЦВ_ПР 29.09.2025 Расширение параметров для передачи элементов из таблицы переменных  
	Если ТипЗнч(ИмяТаблицыИзПеременных) = Тип("Строка") и ЗначениеЗаполнено(ИмяТаблицыИзПеременных) Тогда  
		...
		Если СостояниеVanessaAutomation.ТекущаяФича.ЕстьСекцияПеременные Тогда
			Если ТипЗнч(СостояниеVanessaAutomation.ТекущаяФича.ТаблицыПеременных) = Тип("Соответствие") Тогда
				ДанныеТаблицыИзСекцииПеременных = СостояниеVanessaAutomation.ТекущаяФича.ТаблицыПеременных[НРег(ИмяТаблицыИзПеременных)];
				Если ДанныеТаблицыИзСекцииПеременных <> Неопределено Тогда
					
					// ТаблицаЭлементов - это МАССИВ структур, а не ТаблицаЗначений
					Если ТипЗнч(ТаблицаЭлементов) = Тип("Массив") Тогда
						// Очищаем массив 
						ТаблицаЭлементов.Очистить();  
						// Добавляем заголовочную строку (строка 0)
						ЗаголовочнаяСтруктура = Новый Структура;
						ЗаголовочнаяСтруктура.Вставить("Кол1", "Имя");
						ЗаголовочнаяСтруктура.Вставить("Кол2", "Тип");
						ТаблицаЭлементов.Добавить(ЗаголовочнаяСтруктура);
						
						// Добавляем структуры в массив из данных секции Переменные 
						Для Каждого КлючИЗначение Из ДанныеТаблицыИзСекцииПеременных.Значения Цикл
						    Если ТипЗнч(КлючИЗначение.Значение) = Тип("Структура") Тогда
						        НоваяСтруктура = Новый Структура;
						        НоваяСтруктура.Вставить("Кол1", КлючИЗначение.Значение.Заголовок);
						        НоваяСтруктура.Вставить("Кол2", КлючИЗначение.Значение.Тип);
						        ТаблицаЭлементов.Добавить(НоваяСтруктура);
						    КонецЕсли;
						КонецЦикла;
						
					Иначе
						// Если ТаблицаЭлементов не массив, создаем новый массив
	...
	// Проверяем, что ТаблицаЭлементов готова к использованию
	Если ТипЗнч(ТаблицаЭлементов) <> Тип("Массив") Тогда
		ВызватьИсключение "Таблица элементов не подготовлена для обработки";
	КонецЕсли;
	//---ЦВ_ПР 29.09.2025

	// Один вызов с модифицированной ТаблицаЭлементов
	ПолучитьДанныеКоординатЭлементовПоТаблице(PID, ТаблицаЭлементов, Left, Top, Bottom, Right);
